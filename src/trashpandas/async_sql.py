...
class AsyncSqlStorage:
    ...
    def __init__(self, engine: Union[AsyncEngine, str]) -> None:
...
async def store_df_sql_async(df: DataFrame, table_name: str, engine: AsyncEngine, schema: Optional[str] = None) -> None:
    """Store DataFrame and metadata in database asynchronously.
    ...
    async with engine.begin() as conn:
        ...
        # Store metadata
        await conn.run_sync(lambda sync_conn: metadata.to_sql(
            f'_{table_name}_metadata', sync_conn, if_exists='replace', 
            index=False, schema=schema
        ))


async def load_df_sql_async(table_name: str, engine: AsyncEngine, schema: Optional[str] = None) -> DataFrame:
    """Retrieve DataFrame from database asynchronously.
    ...
    async with engine.begin() as conn:
        df = await conn.run_sync(lambda sync_conn: read_sql_table(
            table_name, sync_conn, schema=schema
        ).astype(types).set_index(indexes))
    
    unname_no_names(df)
    return df


async def delete_table_sql_async(table_name: str, engine: AsyncEngine, schema: Optional[str] = None) -> None:
    """Delete DataFrame and metadata from database asynchronously.
...
async def load_metadata_sql_async(table_name: str, engine: AsyncEngine, schema: Optional[str] = None) -> DataFrame:
    """Retrieve DataFrame metadata from database asynchronously.
...
async def table_names_sql_async(engine: AsyncEngine, schema: Optional[str] = None) -> List[str]:
    """Query database for list of non-metadata table names asynchronously.
    ...
    async with engine.begin() as conn:
        ...
        table_names = await conn.run_sync(lambda sync_conn: inspector.get_table_names(schema=schema))
        return [name for name in table_names if '_metadata' not in name and name[0]!='_']


async def metadata_names_sql_async(engine: AsyncEngine, schema: Optional[str] = None) -> List[str]:
    """Query database for list of metadata table names asynchronously.
...
async def _read_cast_metadata_sql_async(table_name: str, engine: AsyncEngine, schema: Optional[str] = None) -> DataFrame:
    ...
    """
    async with engine.begin() as conn:
        meta = await conn.run_sync(lambda sync_conn: read_sql_table(table_name, sync_conn, schema=schema))
        meta['datatype'] = cast_type(meta['datatype'])
        return meta


async def _first_load_df_sql_async(table_name: str, engine: AsyncEngine, schema: Optional[str] = None) -> DataFrame:
    """Load a SQL table that has no metadata stored, create and store metadata asynchronously.
    ...
    """
    async with engine.begin() as conn:
        df = await conn.run_sync(lambda sync_conn: read_sql_table(table_name, sync_conn, schema=schema))
    
    await store_df_sql_async(df, table_name, engine, schema)
    return df


async def _get_table_async(table_name: str, engine: AsyncEngine, schema: Optional[str] = None) -> Table:
    ...
    """
    async with engine.begin() as conn:
        metadata = await conn.run_sync(lambda sync_conn: MetaData(bind=sync_conn, schema=schema))
        return await conn.run_sync(lambda sync_conn: Table(
            table_name, metadata, autoload=True, schema=schema
        ))
