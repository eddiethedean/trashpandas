...
class AsyncCsvStorage(IFileStorage):
    ...
    async def __aexit__(self, exc_type: Optional[Exception], exc_val: Optional[Exception], exc_tb: Optional[object]) -> None:
        """Exit async context manager."""
        pass

    async def store(self, df: DataFrame, table_name: str, schema: Optional[str] = None) -> None:
    ...
    async def load(self, table_name: str, schema: Optional[str] = None) -> DataFrame:
    ...
    async def delete(self, table_name: str, schema: Optional[str] = None) -> None:
    ...
    async def load_metadata(self, table_name: str, schema: Optional[str] = None) -> DataFrame:
        ...
        """
        return await load_metadata_csv_async(table_name, str(self.path), self.compression)

    async def table_names(self, schema: Optional[str] = None) -> List[str]:
    ...
    async def metadata_names(self, schema: Optional[str] = None) -> List[str]:
    ...
    async def store_many(self, dataframes: Dict[str, DataFrame], schema: Optional[str] = None) -> None:
        ...
        """
        tasks = [
            self.store(df, table_name, schema) 
            for table_name, df in dataframes.items()
        ]
        await asyncio.gather(*tasks)
    
    async def load_many(self, table_names: List[str], schema: Optional[str] = None) -> Dict[str, DataFrame]:
        ...
        """
        tasks = [self.load(table_name, schema) for table_name in table_names]
        results = await asyncio.gather(*tasks)
        return dict(zip(table_names, results))
    
    async def delete_many(self, table_names: List[str], schema: Optional[str] = None) -> None:
...
class AsyncPickleStorage(IFileStorage):
    ...
    async def store(self, df: DataFrame, table_name: str, schema: Optional[str] = None) -> None:
        ...
        """
        await store_df_pickle_async(df, table_name, str(self.path), self.file_extension, self.compression)
    ...
    async def load(self, table_name: str, schema: Optional[str] = None) -> DataFrame:
        ...
        Returns:
            The stored DataFrame
        """
        return await load_df_pickle_async(table_name, str(self.path), self.file_extension, self.compression)
...
# Async CSV functions
async def store_df_csv_async(df: DataFrame, table_name: str, path: str, compression: Optional[str] = None) -> None:
    """Store DataFrame and metadata as CSV files asynchronously."""
    def _store():
        ...
        df_copy.to_csv(csv_path, compression=compression)
...
async def load_df_csv_async(table_name: str, path: str, compression: Optional[str] = None) -> DataFrame:
    """Retrieve DataFrame from CSV file asynchronously."""
    def _load():
        csv_path = _get_csv_path_async(table_name, path, compression)
        metadata_path = _get_metadata_csv_path_async(table_name, path, compression)
...
async def delete_table_csv_async(table_name: str, path: str, compression: Optional[str] = None) -> None:
    """Delete DataFrame and metadata CSV files asynchronously."""
    def _delete():
        csv_path = _get_csv_path_async(table_name, path, compression)
        metadata_path = _get_metadata_csv_path_async(table_name, path, compression)
...
async def load_metadata_csv_async(table_name: str, path: str, compression: Optional[str] = None) -> DataFrame:
    """Retrieve DataFrame metadata from CSV file asynchronously."""
    def _load():
...
async def store_df_pickle_async(df: DataFrame, table_name: str, path: str, file_extension: str = '.pickle', compression: Optional[str] = None) -> None:
    """Store DataFrame as pickle file asynchronously."""
    def _store():
        pickle_path = _get_pickle_path_async(table_name, path, file_extension, compression)
        df.to_pickle(pickle_path, compression=compression)
    
    await asyncio.get_event_loop().run_in_executor(None, _store)


async def load_df_pickle_async(table_name: str, path: str, file_extension: str = '.pickle', compression: Optional[str] = None) -> DataFrame:
    """Retrieve DataFrame from pickle file asynchronously."""
    def _load():
        pickle_path = _get_pickle_path_async(table_name, path, file_extension, compression)
...
async def delete_table_pickle_async(table_name: str, path: str, file_extension: str = '.pickle') -> None:
    """Delete DataFrame pickle file asynchronously."""
    def _delete():
...
async def table_names_pickle_async(path: str, file_extension: str = '.pickle') -> List[str]:
    """Get list of stored table names asynchronously."""
    def _get_names():
        filenames = os.listdir(path)
...
# Helper functions
def _get_csv_path_async(table_name: str, path: str, compression: Optional[str] = None) -> str:
...
def _get_metadata_csv_path_async(table_name: str, path: str, compression: Optional[str] = None) -> str:
...
def _get_pickle_path_async(table_name: str, path: str, file_extension: str = '.pickle', compression: Optional[str] = None) -> str:
...
def _read_cast_metadata_csv_async_sync(table_name: str, path: str, compression: Optional[str] = None) -> DataFrame:
...
def _first_load_df_csv_async_sync(table_name: str, path: str, compression: Optional[str] = None) -> DataFrame:
    """Load a CSV that has no metadata stored, create and store metadata (sync version for async)."""
    csv_path = _get_csv_path_async(table_name, path, compression)
    ...
    name_no_names(df_copy)
    metadata = df_metadata(df_copy)
    metadata.to_csv(csv_path_meta, index=False, compression=compression)
    return df
